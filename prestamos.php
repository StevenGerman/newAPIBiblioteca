...
function getRequest($pdo) {
    ...
    if (isset($_GET['idPrestamo'])) {
        $sql = $pdo->prepare("SELECT * FROM prestamos WHERE idPrestamo=:idPrestamo");
        $sql->bindValue(':idPrestamo', $_GET['idPrestamo']);
        $sql->execute();
        $sql->setFetchMode(PDO::FETCH_ASSOC);
        header("HTTP/1.1 200 OK");
        echo json_encode($sql->fetchAll());
    }else {
        $sql = $pdo->prepare("SELECT p.idPrestamo,p.presFechaDev,p.presFechaSal,p.presObservacion,p.personaID,p.libroID, personas.perNombre, personas.perApellido,l.libTitulo FROM prestamos as p INNER JOIN personas on p.personaID = personas.idPersona INNER JOIN libros as l ON p.libroID = l.idLibro");
        $sql->execute();
        $sql->setFetchMode(PDO::FETCH_ASSOC);
        header("HTTP/1.1 200 OK");
        echo json_encode($sql->fetchAll());
